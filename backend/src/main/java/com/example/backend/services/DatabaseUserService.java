...
import com.example.backend.components.JwtUtilities;
...
public class DatabaseUserService implements UserService{
    ...
    @Autowired
    public DatabaseUserService(UserRepository userRepository, JwtUtilities jwtUtilities, EmailSenderService emailSenderService, PasswordEncoder passwordEncoder) {
    ...
    public ResponseEntity<Responses> register(User user) {
        ...
        if (userRepository.findByUsername(user.getUsername()).isPresent()) {
            return ResponseEntity.status(400).body(new Responses("username","User already exists!"));
        }
        ...
        emailSenderService.sendEmail(user.getEmail(), activationCode);

        return ResponseEntity.ok(new Responses("success",user.getUsername() +
                ", you were successfully registered! Now you can log in and have to activate your account to full access."));
    }
    ...
    public ResponseEntity<?> activate(String username, String activationCode) {
        Optional<User> user = findByUsernameOrEmail(username, username);

        if (user.get().getActive() == true) {
            return ResponseEntity.badRequest().body(new Responses("error", "Your account is already activated!"));
        }

        if (!user.get().getActivationCode().equals(activationCode)) {
            return ResponseEntity.badRequest().body(new Responses("error", "Wrong activation code!"));
        }
        ...
        return ResponseEntity.ok(new Responses("success", "Your account was activated!"));
    }
